...
@RestController
@Api(tags = {"accessors"})
@RequestMapping(path = "/api/v1.1/acls/{aclId}/accessors", produces = { "application/json" })
public class AccessorRestControllerV1_1 extends BaseAccessorRestController {
	@ApiOperation(response = Accessor.class, value = "Retrieves all Accessors of the ACL", responseContainer = "List")
	@GetMapping()
	public ResponseEntity<Page<Accessor>> getAllAccessors(@PathVariable String aclId, Pageable pageable) {
		log.debug("Start getting a page {} (size {}) of all Accessors for ACL (id='{}')", pageable.getPageNumber(),
				pageable.getPageSize(), aclId);
		
		Page<Accessor> page = Page.of(doGetAll(aclId), pageable.getPageNumber(), pageable.getPageSize());
